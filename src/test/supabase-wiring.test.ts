...
describe('Supabase production wiring', () => {
  ...
  it('defines secure ai-chef Edge Function with supported actions', () => {
    ...
    expect(edgeFunction).toContain('repair_menu_plan')
    expect(edgeFunction).toContain('contextual_suggestion')
    expect(edgeFunction).toContain('aiCopilotResponseSchema')
    expect(edgeFunction).toContain('validateCopilotOutput')
    expect(edgeFunction).toContain('page_context')
    ...
    expect(edgeFunction).toContain('validateRepairedMenuPlanOutput')
    expect(edgeFunction).toContain('Repair only meals marked review_needed or blocked')
    expect(edgeFunction).toContain('safe_slots_were_locked')
    ...
    expect(edgeFunction).not.toContain('?key=${apiKey}')
    expect(edgeFunction).not.toContain("admin.from('recipe_ingredients').select('*'),")
    expect(edgeFunction).not.toContain("from('menu_plans').insert")
    expect(edgeFunction).not.toContain("from('menu_plan_items').insert")
  ...
  it('manages BYOK keys only through an authenticated Edge Function', () => {
    ...
    expect(keyManager).toContain('Key valid. Click Save to enable.')
    expect(keyManager).toContain('configured: false')
    expect(keyManager).toContain('is_enabled: false')
    expect(keyManager).toContain('isEnabled: true')
    ...
    expect(keyManager).toContain("'x-goog-api-key': apiKey")
    expect(keyManager).not.toContain('?key=${apiKey}')
