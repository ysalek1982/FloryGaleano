...
describe('Supabase production wiring', () => {
  ...
  it('defines secure ai-chef Edge Function with supported actions', () => {
    ...
    expect(edgeFunction).toContain('normalizeSuggestionCategory')
    expect(edgeFunction).toContain('inferCategoryCode')
    expect(edgeFunction).toContain('Missing category_code mapped to other.')
    ...
    expect(edgeFunction).toContain('generate_validated_menu_plan')
    expect(edgeFunction).toContain('suggest_substitutions')
    expect(edgeFunction).toContain('optimize_freezer_usage')
    expect(edgeFunction).toContain('menuPlanResponseSchema')
    expect(edgeFunction).toContain('getCandidateRecipes')
    expect(edgeFunction).toContain('validateMenuPlanOutput')
    expect(edgeFunction).toContain("admin.from('recipe_ingredients').select('*').in('recipe_id', recipeIds)")
    expect(edgeFunction).toContain('candidate_recipe_count')
    expect(edgeFunction).toContain('gemini_rate_limited')
    expect(edgeFunction).toContain('validateAiOutput')
    expect(edgeFunction).not.toContain("admin.from('recipe_ingredients').select('*'),")
  ...
  it('manages BYOK keys only through an authenticated Edge Function', () => {
    ...
    expect(keyManager).toContain("return json({ error: 'Unauthorized.' }, 401)")
    expect(keyManager).toContain('publicMetadata')
    expect(keyManager).toContain('clearStoredKey')
    expect(keyManager).toContain('friendlyGeminiError')
    expect(keyManager).toContain('Gemini quota or rate limit was reached')
    expect(keyManager).toContain("'rate_limited'")
    expect(keyManager).toContain('retry_after_seconds')
    expect(keyManager).toContain('listGeminiModels')
    expect(keyManager).toContain('https://generativelanguage.googleapis.com/v1beta/models?key=')
    expect(keyManager).toContain('keepForRetry')
    expect(keyManager).not.toContain('localStorage')
    expect(keyManager).not.toContain('AIza')
  })

  it('adds database support for retryable Gemini rate limits', () => {
    const migration = read('supabase/migrations/20260524223000_ai_key_rate_limited_status.sql')
    expect(migration).toContain("'rate_limited'")
    expect(migration).toContain('last_rate_limited_at')
